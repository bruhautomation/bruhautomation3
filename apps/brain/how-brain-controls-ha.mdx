...
**Accepting a [proposal](/brain/proposals/#saying-yes-and-taking-it-back)** appends one
automation to `automations.yaml` from your press, with no Claude run behind it (1.44.0). The
file is snapshotted into the same edit journal first, the automation is only settled once
Home Assistant is verifiably running it, the write is reverted if it is not, and both the
toast and `brain undo` take it back.
