...
### One write happens without Claude

Everything above is Claude with a model in the loop, on your instruction. There is exactly one
exception, since 1.44.0: accepting a [proposal](/brain/proposals/#saying-yes-and-taking-it-back)
appends one automation to `automations.yaml` from your press, with no Claude run behind it. It
is the narrowest thing that can do that job — the file is snapshotted into the same edit
journal first, the automation is only settled once Home Assistant is verifiably running it,
the write is reverted if it is not, and both the toast and `brain undo` take it back.
