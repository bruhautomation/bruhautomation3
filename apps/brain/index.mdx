---
title: brAIn
description: Your house already has nerves. Now give it a brAIn. A Home Assistant add-on that runs Claude Code inside HA, sees your whole system, can change any of it, and builds a permanent memory of your house.
template: splash
hero:
  tagline: "Your house already has nerves. Now give it a brAIn. Claude plus a suite of new tools hands it the keys. Stop programming your house — let it think."
  ...
  image:
    light: ./brain-logo-onlight.svg
    dark: ./brain-logo-ondark.svg
  actions:
    - text: Quick Start
...
![The brAIn Insights tab: an ask bar across the top, tag filters, and four insight cards — one answering "Why did the hallway light come on at 3 am?" with a trace timeline, one showing a month of electricity by device as a bar chart](./images/panel-insights.webp)
...
![The brAIn Findings tab, headed "What brAIn thinks is broken", listing thirteen orphaned entities, two loft sensors that never rejoined after a Zigbee channel change, and a dead back-door battery sensor — each with Fix it, Discuss, I fixed it, Remind me later and Ignore](./images/panel-findings.webp)

Press **Fix it** and Claude makes the change, verifies it, and reports what it touched.
Press **Discuss** to talk it over first, changing nothing. **I fixed it** and **✕ Wrong** are
endings: the answer becomes a fact in memory, and the row is gone for good rather than
joining a pile of dismissed cards. Both take an optional one-line note, and the note is the
half that teaches — *"that sensor always reads on, it's not stuck"* becomes a standing fact
about your house, so the next run doesn't make the same mistake in new words.
...
Two of the things it offers on the [Proposals](/brain/proposals/) tab start with you rather
than with a pattern it found:

- **[One-off intents](/brain/intents/)** — *"turn the porch light off when the guests leave"*.
  Type it into the ask bar, or call `brain.intent` from a voice command. brAIn writes an
  ordinary automation that **runs once and switches itself off**, shows you a replay of the
  trigger before you accept it, and then leaves it in your automations saying it fired until
  you press Remove. Nothing removes an automation for you.
...
![The brAIn Memory tab: two guesses awaiting a yes/no answer, three discoveries waiting to be filed, and the home memory document below with sections on the household, the house, and device quirks](./images/panel-memory.webp)
...
![The brAIn panel on a phone, showing the two-row top bar with all five tabs named beneath the status row](./images/panel-phone.webp)
