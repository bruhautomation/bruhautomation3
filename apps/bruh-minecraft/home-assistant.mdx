...
| `sensor.…_tps_1m/5m/15m` | Server tick rate (Paper/Purpur/Folia) — the lag early-warning |
...
There's also a **notify platform**: `notify.bruh_minecraft_broadcast` takes a `message` and broadcasts it in-game, so anything that can send a notification — doorbells, timers, dinner — can reach the players.
...
Paired with a [brAIn](/brain/) voice assistant, *"tell everyone dinner's ready"* does this hands-free:
