...
├── preview/           # meshes tessellated from cad/ so the viewer can show
│                      #   parts that were never exported to STL — generated by
│                      #   site/scripts/build-cad-previews.py, never hand-edited
...
A part that only exists as CAD still gets a 3D preview: `site/scripts/build-cad-previews.py`
tessellates every `cad/*.step` into `preview/*.stl`, and the site's viewer loads
those alongside the hand-exported STLs, labelled *from CAD*. Run it after adding
or changing a STEP file — it needs OpenCascade, so it is not part of the site
build, and its output is committed:
...
deviation or better, which is finer than a 0.4 mm nozzle resolves — it is moved
into `models/`, so a project that only ever shipped CAD still has something you
can slice (the viewer loads it from there, so nothing is kept in `preview/`
twice). A part that had to be tessellated coarser to fit the
