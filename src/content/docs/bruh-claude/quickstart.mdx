...
4. **Set up the integration.** After the restart, HA discovers it automatically — accept the prompt in **Settings → Devices & Services**. This registers the conversation agent, the usage-limit sensors, the assist-health binary sensor, and the `bruh_claude.send_prompt` / `run_task` / `run_insight` / `clear_conversation` services.
