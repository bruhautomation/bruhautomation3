...
3. **Restart Home Assistant** — required on first install so HA loads the BRUH Claude integration: **Settings → System → Restart**.
...
5. **Sign in with Anthropic.** Open the **BRUH Claude** entry in the HA sidebar and complete the OAuth login in the terminal. Credentials are stored in the add-on's persistent volume — they survive restarts.
