...
**Power Tools are not a second pair of hands — they're verbs installed into HA itself.** Registering the 56 admin operations as real `brain.*` services (instead of private Claude abilities) is a deliberate choice with three payoffs:
...
They are two commands, split by what they act on: **`brain`** for brAIn's own faculties (`memory`, `learn`, `ask`, `undo`, `doctor`) and **`ha`** for Home Assistant operations (`log`, `reload`, `entity`, `service`, `addon`, `notify`, `share`, `check`, `context`). See [the CLI](/brain/cli/).
