...
Inside a Claude Code session, two shortcuts do the same work without leaving the conversation:

- **`/learn [topic]`** — start a study session and watch it work, correcting it mid-flight.
- **`/memory`** — show what it knows right now.

## There is no backup command

brAIn doesn't version your configuration; **Home Assistant's own backups** do that job, whole-system and restorable. What it keeps instead is an edit journal — a snapshot of any file *before* Claude writes to it — which `brain undo` restores from. See [Undo](/brain/reference/#undo).
